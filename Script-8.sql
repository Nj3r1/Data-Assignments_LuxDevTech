...
-- 8. Products in 'Electronics' category
SELECT * FROM assignment.products WHERE category = 'Electronics';

-- 9. Highest price
SELECT MAX(price) AS highest_price FROM assignment.products;

-- 10. Number of sales per product
SELECT product_id, COUNT(*) AS num_sales FROM assignment.sales GROUP BY product_id;
...
-- 13. Customers who purchased products priced > 1000
...
-- 14. Join Sales and Products — product name and total sales amount
...
JOIN assignment.customers b 
  ON a.membership_status = b.membership_status AND a.customer_id < b.customer_id;

-- 18. Total number of sales per product
SELECT p.product_name, COUNT(s.sale_id) AS total_sales
FROM assignment.sales s
JOIN assignment.products p ON s.product_id = p.product_id
GROUP BY p.product_name;

-- 19. Products with stock quantity < 10
...
-- 21. Customers who purchased 'Electronics' or 'Appliances'
SELECT DISTINCT c.customer_id, c.first_name, c.last_name
FROM assignment.customers c
JOIN assignment.sales s ON c.customer_id = s.customer_id
JOIN assignment.products p ON s.product_id = p.product_id
...
-- 22. Total sales amount per product grouped by product name
...
-- 24. Customers with the highest total sales in 2023
SELECT c.customer_id, c.first_name, c.last_name, SUM(s.total_amount) AS total_spent
FROM assignment.customers c
JOIN assignment.sales s ON c.customer_id = s.customer_id
WHERE EXTRACT(YEAR FROM s.sale_date) = 2023
...
-- 26. Total number of customers who purchased products worth > 500
...
-- 27. Total sales made by 'Gold' membership customers
...
-- 28. Products with low stock (< 10) from Products and Inventory
SELECT p.product_name, i.stock_quantity
FROM assignment.products p
JOIN assignment.inventory i ON p.product_id = i.product_id
WHERE i.stock_quantity < 10;

-- 29. Customers who purchased more than 5 products (total quantity > 5)
...
-- 33. Products that have been sold but have < 5 units left in inventory
SELECT p.product_name, i.stock_quantity
FROM assignment.products p
JOIN assignment.inventory i ON p.product_id = i.product_id
...
-- 34. Total sales per product ordered by highest sales
SELECT p.product_name, SUM(s.total_amount) AS total_sales
FROM assignment.sales s
JOIN assignment.products p ON s.product_id = p.product_id
GROUP BY p.product_name
...
-- 35. Customers who bought products within 7 days of their registration date
SELECT DISTINCT c.customer_id, c.first_name, c.last_name
FROM assignment.customers c
JOIN assignment.sales s ON c.customer_id = s.customer_id
WHERE s.sale_date BETWEEN c.registration_date AND c.registration_date + INTERVAL '7 days';

-- 36. Sales joined with Products filtered by price between 100 and 500
...
-- 39. Products with highest and lowest stock in a single result set
SELECT product_name, stock_quantity, 'Highest Stock' AS label
FROM assignment.products
WHERE stock_quantity = (SELECT MAX(stock_quantity) FROM assignment.products)
UNION ALL
SELECT product_name, stock_quantity, 'Lowest Stock'
FROM assignment.products
WHERE stock_quantity = (SELECT MIN(stock_quantity) FROM assignment.products);

-- 40. Products with 'Phone' in the name and their total sales
SELECT p.product_name, SUM(s.total_amount) AS total_sales
FROM assignment.products p
JOIN assignment.sales s ON p.product_id = s.product_id
...
-- 41. Gold customers: total sales amount and product names
...
-- 44. Products sold but still have stock remaining
SELECT p.product_name, i.stock_quantity
FROM assignment.sales s
JOIN assignment.products p ON s.product_id = p.product_id
JOIN assignment.inventory i ON p.product_id = i.product_id
...
-- 45. Top 5 customers by total purchases
SELECT c.customer_id, c.first_name, c.last_name, SUM(s.total_amount) AS total_spent
FROM assignment.customers c
JOIN assignment.sales s ON c.customer_id = s.customer_id
GROUP BY c.customer_id, c.first_name, c.last_name
...
-- 46. Total unique products sold in 2023
...
-- 50. Products sold more than 100 times with price > 200
